$(document).ready(function () {
    $('body').on('click', '[id^="btnTaoDonBuffet"]', function () {
        var btn = $(this);
        ...
        $.ajax({
            ...
            url: $('#requestPath').val() + "admin/lienhedatban/opentaodonbuffet",
            data: formData,
            dataType: 'html',
            type: 'POST',
            processData: false,
            ...
            if (ketqua.indexOf("Chi tiết lỗi") != -1) {
                btn.html('<i class="bi bi-plus-circle me-2"></i> Tạo đơn Buffet');
                ...
                $('body').find('[id="TaoDonBuffetModalPartial"]').replaceWith(ketqua);
                $('body').find('[id="TaoDonBuffetModal"]').modal('toggle');
            }
        });
    });
});