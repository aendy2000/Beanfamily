$(document).ready(function () {
    ...
    $('body').on('click', '[id="xoahangloat"]', function () {
        ...
        if (soluong > 0) {
            Swal.fire({
                title: 'Xóa bỏ?',
                text: 'Bạn có chắc muốn xóa ' + soluong + ' sản phẩm đã chọn?',
                icon: "question",
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({error: function (a, xhr, c) {if (a.status == 403 && a.responseText.indexOf("SystemLoginAgain") != -1) {window.location.href = $('body').find('[id="requestPath"]').val() + "admin/dangnhap/logout";}},
                        url: $('#requestPath').val() + "admin/sanphamvuonraubean/xoahangloat",
                        data: formData,
                        dataType: 'html',
                        type: 'POST',
                        processData: false,
                        ...
                        if (ketqua == "SUCCESS") {
                            Swal.fire({
                                title: "Thành công!",
                                text: soluong + ' sản phẩm được chọn đã được xóa bỏ.',
                                icon: "success"
                            }).then(() => {
